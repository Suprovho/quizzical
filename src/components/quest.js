export default function Quest(props) {
  ...
  const options = props.question.options.map((option, index) => (
    <button
      key={index}
      dangerouslySetInnerHTML={{ __html: option }} //denger... work as a innerhtml js function inform react as  html tag
      onClick={(event) => props.selectAnswers(event, props.id, index)}
